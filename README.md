...
| from                | имя пользователя               | user...
| text                | текст самого сообщения         | -
| day_of_week         | день недели отправки сообщения | -
| year_month          | год и месяц отправки сообщения | -
| time_message        | время отпрвки сообщения        | 13:01:04
| top                 | входят в топ-10 или не входят  | Топ-10, не входят
| len_messages        | длина одного сообщения         | - 
...
```python
d = {'date':[], 'from':[], 'text':[]}``` # каждый ключ соотвествует пустому списку, который будет заполняться данными в процессе заданного цикла
